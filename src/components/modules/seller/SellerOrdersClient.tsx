...
export default function SellerOrdersClient() {
    ...
    useEffect(() => {
        ...
        const load = async () => {
            ...
            setError(null);
            try {
                const { ok, data } = await orderService.getSellerOrders();
                if (!ok) {
                ...
                if (!cancelled) setError("Something went wrong while loading seller orders.");
            } finally {
    ...
    const onUpdateStatus = async (order: OrderWithOptionalUser) => {
        ...
        setError(null);
        try {
            const { ok, data } = await orderService.updateSellerOrderStatus(order.id, nextStatus);
            if (!ok) {
    ...
    if (!canManage) {
        return (
            <main className="mx-auto min-h-[70vh] max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
                <div className="rounded-2xl border border-red-200 bg-red-50 px-4 py-3 text-red-600">
                    You do not have permission to manage seller orders.
                </div>
            </main>
        );
    }
    ...
    return (
        <main className="mx-auto min-h-[70vh] max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
            <section className="mb-7 rounded-3xl border border-border/70 bg-card/85 px-6 py-7 sm:px-8">
                <h1 className="text-3xl font-bold text-emerald-700 sm:text-4xl">Seller Orders</h1>
                <p className="mt-2 text-sm text-muted-foreground">Review and update order statuses from one place.</p>
                        ...
                        <Card key={order.id} className="rounded-2xl border-border/70 bg-card/95">
                            <CardContent className="space-y-4 p-6">
                                ...
                                <div className="grid grid-cols-1 gap-3 sm:grid-cols-[1fr_auto] sm:items-end">
                                    ...
                                    >
                                        {savingOrderId === order.id ? "Updating..." : "Save Status"}
                                    </Button>
                                </div>
                            </CardContent>
                        </Card>
